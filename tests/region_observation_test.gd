...
func _ready() -> void:
	...
	var building_label: Label = view.find_child("Label", true, false)
	_expect(building_label != null and "测试观测站" in building_label.text, "首次创建的建筑视觉没有立即写入名称与状态")
	view.set_observed_zone(1)
	view.refresh_from_state(GameState.get_state())
	await get_tree().process_frame
	_expect(view.get_building_visual_ids().is_empty(), "切换到无建筑区域后仍保留上一地区建筑视觉")
	var other_building = EntityScript.GameBuilding.new(9002, "异区测试营地", "algae_foraging", 1)
	GameState.entities.add_building(other_building)
	GameState.planet_zones.add_building_to_zone(1, other_building.id)
	view.refresh_from_state(GameState.get_state())
	_expect(view.get_building_visual_ids() == [9002], "区域切换后的建筑视觉没有按新区域实体重建")
	var other_label: Label = view.find_child("Label", true, false)
	_expect(other_label != null and "异区测试营地" in other_label.text, "区域切换后建筑标签仍显示上一地区内容")

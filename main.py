...
def run_game_loop(config: dict, screen_manager: ScreenManager, screen: pygame.Surface):
    ...
    while running:
        ...
        # 仅在用户从设置界面应用时同步时间倍率（避免覆盖游戏内速度按钮）
        if screen_manager.global_state.get('_settings_apply_pending', False):
            ...
            screen_manager.global_state['_settings_apply_pending'] = False
